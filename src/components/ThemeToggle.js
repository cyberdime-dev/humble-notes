...
export default function ThemeToggle() {
  ...
  const toggleTheme = () => {
    ...
    if (newTheme) {
      ...
      console.log('Added dark class, saved dark theme');
    } else {
      document.documentElement.classList.remove('dark');
      localStorage.setItem('theme', 'light');
      console.log('Removed dark class, saved light theme');
    }
  };
  ...
  if (!mounted) {
    return (
      <button
        className="absolute top-6 right-6 p-3 rounded-2xl bg-custom-button backdrop-blur-sm border border-zinc-200 transition-all duration-200 shadow-sm"
